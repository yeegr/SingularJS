...
import Totp, { ITotp } from './_shared/totp/TotpModel'
import SMS from '@system/sms'
import Emailer from '@system/emailer'
  ...
  .then((totp: ITotp) => {
    if (totp) {
      ...
      if (totp.action === CONST.USER_ACTIONS.COMMON.UPDATE) {
        ...
        return UserModel
          .findByIdAndUpdate((req.user as IUser)._id!, q, {new: true})
      } else {
        return UserModel
        .findOne(q)
        .then((user: IUser) => {
