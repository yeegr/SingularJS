...
class ActionRouter {
  ...
  public create = (req: Request, res: Response) => {
    ...
    if (!creator || !creatorRef) {
      res.status(422).json({ code: ERRORS.ACTION.ACTION_CREATOR_REQUIRED })
    } else if (!targetRef || !target) {
      res.status(422).json({ code: ERRORS.ACTION.ACTION_TARGET_NOT_SPECIFIED })
    } else if (CONST.ACTION_TARGETS_ENUM.indexOf(targetRef) < 0) {
      res.status(400).json({ code: ERRORS.ACTION.ACTION_TARGET_NOT_FOUND })
    } else if (!action) {
      res.status(422).json({ code: ERRORS.ACTION.ACTION_TYPE_REQUIRED })
        ...
        .then((doc: any) => {
          if (doc) {
            if ((<any>creator).equals(doc.creator)) {
              res.status(422).json({ code: ERRORS.ACTION.ACTION_NOT_AUTHORIZED})
            } else {
              switch (action) {
                ...
                case CONST.USER_ACTIONS.CONSUMER.UNFOLLOW:
                  ...
                  .then((act: IAction) => {
                    if (!act) {
                      res.status(404).json({ code: ERRORS.ACTION.CANNOT_UNDO_NON_ACTION })
                    } else {
                      res.status(res.statusCode).send()                      
                      this.logger(log)
                    }
                ...
                default:
                  res.status(422).json({ code: ERRORS.ACTION.UNABLE_TO_PERFORM_ACTION })
                break
              }
            }
          } else {
            res.status(404).json({ code: ERRORS.ACTION.ACTION_TARGET_NOT_FOUND })
          }
        })
        .catch((err: Error) => {
          new Err(res, err, log)
